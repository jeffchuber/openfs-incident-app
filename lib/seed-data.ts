// Works with any Vfs — just needs write(path, content)
interface Writable {
...
export async function seedIncidentData(client: Writable): Promise<void> {
  for (const [path, content] of seeds) {
    await client.write(path, content);
  }
}

/** Seed only the in-memory paths (runbooks + scratch) — for prod mode where
 *  Postgres/S3 data is already persisted but memory backend resets on restart. */
export async function seedMemoryPaths(client: Writable): Promise<void> {
  for (const [path, content] of seeds) {
    if (path.startsWith("/runbooks/") || path.startsWith("/scratch/")) {
      await client.write(path, content);
    }
  }
}
